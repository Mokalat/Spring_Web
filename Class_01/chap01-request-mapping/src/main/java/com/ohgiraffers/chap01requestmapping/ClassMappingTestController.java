...
public class ClassMappingTestController {
    ...
    public String registOrderPost(Model model){
        model.addAttribute("message","post 방식의 주문 등록용 핸들러");
        return "mappingResult";
    }
    ...
    // orders/1 : get pathVariable
    // orders?name=아메리카노 : get
    /*
        path variable
        @PathVariable 어노테이션을 이용해 요청 path로 부터 변수를 받아올 수 있다.
        path variavle로 전달 되는 {변수명} 값은 반드시 매개변수명과 동일해야 한다.
    ...
    */
    ...
    public String selectOrderDetail(Model model,@PathVariable("orderNo") int orderNo){
        model.addAttribute("orderNo", orderNo + "번 주문 상세 내용 조회용 핸들러");
        return "mappingResult";
    }
    ...
    /*
        그 외의 다른 요청
        @RequestMapping 이노테이션에 아무런 url을 설저하지 않으면 요청 처리에 대한 핸들러 메소드가
